...
def phase_1(t, X, wind_std, wind_angle):
    ...
    F_align = Trans.launcher_to_align(q, Vectors.gravity_launcher(t)) + Vectors.thrust_align(t) + Vectors.Ft_align(rx_launcher, airflow_align, roll_align)
...
def phase_2(t, X, wind_std, wind_angle):
    ...
    F_launcher = Vectors.gravity_launcher(t) + Trans.align_to_launcher(q, Vectors.thrust_align(t) + Vectors.Ft_align(rx_launcher, airflow_align, roll_align) + Vectors.Fn_align(rx_launcher, airflow_align, roll_align))
    diff_p_launcher = F_launcher

    I = CV.inertia(t)
    diff_ang_v_align = inv(I) @ (Vectors.Tn_align(t, rx_launcher, airflow_align, roll_align) - cross(ang_v_align, I @ ang_v_align))
...
def solve_all(wind_std, wind_angle):
    ...
    if OPEN_PARACHUTE == 0:
        ...
        #Phase3 : apogee -> landing
        solve_phase_3 = solve(phase_2, wind_std, wind_angle, X0_phase_3, t_start_phase_3, landing)
        ...
        q1, q2_3 = q[:len(t1)].copy(), q[len(t1):].copy()
        ...
        Fn_align2_3 = Fn_align[len(t1):].copy()
        Fn_norm = norm(Fn_align, axis=1)

        Ft_align = Vectors.Ft_align(altitude, airflow_align, roll)
        Ft_align1, Ft_align2_3 = Ft_align[:len(t1)].copy(), Ft_align[len(t1):].copy()
        ...
        a_launcher = np.concatenate([a_launcher1, a_launcher2_3])
    ...
    if OPEN_PARACHUTE == 1:
        ...
        #quaternion配列に変換
        q = qtn.as_quat_array(np.array([Xt[0], Xt[1], Xt[2], Xt[3]]).T)
        q1, q2 = q[:len(t1)].copy(), q[len(t1):len(t1) + len(t2)].copy()
        ...
        down_range = np.array([np.sqrt(r_launcher[i, 1]**2 + r_launcher[i, 2]**2) for i in range(len(r_launcher)) ])
        altitude = r_launcher[:, 0]
        altitude3 = altitude[len(t1) + len(t2):].copy()

        v_launcher = np.array([Xt[4] / CV.mass(t), Xt[5] / CV.mass(t), Xt[6] / CV.mass(t)]).T
        v_launcher3 = v_launcher[len(t1) + len(t2):].copy()
        ...
        wind_launcher = Vectors.wind_launcher(altitude, wind_std, wind_angle)
        wind_launcher3 = wind_launcher[len(t1) + len(t2):].copy()
        ...
        airflow_launcher = wind_launcher - v_launcher
        airflow_align = wind_align - v_align
        airflow_align3 = airflow_align[len(t1) + len(t2):].copy()
        airflow_norm = norm(airflow_launcher, axis=1)

        Fn_align = Vectors.Fn_align(altitude, airflow_align, roll)
        Fn_align[:len(t1)][True] = np.array([0.0, 0.0, 0.0])
        Fn_align2 = Fn_align[len(t1):len(t1) + len(t2)].copy()
        Fn_norm = norm(Fn_align, axis=1)

        Ft_align = Vectors.Ft_align(altitude, airflow_align, roll)
        Ft_align1, Ft_align2 = Ft_align[:len(t1)].copy(), Ft_align[len(t1):len(t1) + len(t2)].copy()
        Ft_norm = norm(Ft_align, axis=1)
        ...
        a_launcher1 = Trans.align_to_launcher(q1, a_align1)
        a_launcher2 = (Vectors.gravity_launcher(t2) + Trans.align_to_launcher(q2, Vectors.thrust_align(t2) + Ft_align2 + Fn_align2)) / np.array([CV.mass(t2), CV.mass(t2), CV.mass(t2)]).T
        a_launcher3 = (Vectors.gravity_launcher(t3) + Trans.parachute_align_to_launcher(v_launcher3, wind_launcher3, Vectors.Fd_parachute(altitude3, airflow_align3))) / np.array([CV.mass(t3), CV.mass(t3), CV.mass(t3)]).T
        a_launcher = np.concatenate([a_launcher1, a_launcher2, a_launcher3])
