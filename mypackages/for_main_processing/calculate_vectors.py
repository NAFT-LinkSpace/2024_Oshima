...
def Cp_body(time, airflow_align, roll):

    alpha = CV.alpha_airflow(airflow_align)

    Cp = np.array([CV.Cp(alpha, roll) - CV.Cg(time), np.zeros_like(time), np.zeros_like(time)]).T
...
def Fn_align(altitude, airflow_align, roll):
    alpha = CV.alpha_airflow(airflow_align)
    ex = np.array([1, 0, 0])
    ...
    if airflow_align.ndim > 1:
        ...
        Fn_align_yz = CV.dp(altitude, airflow_align)[:, None] * S_BODY * CV.Cn(alpha, roll)[:, None] * direction_yz
    else:
        if norm(airflow_yz) != 0:
            direction_yz = airflow_yz / norm(airflow_yz)
        else:
            direction_yz = np.array([0, 0, 0])
        Fn_align_yz = CV.dp(altitude, airflow_align) * S_BODY * CV.Cn(alpha, roll) * direction_yz

    return Fn_align_yz

def Ft_align(altitude, airflow_align, roll):
    alpha = CV.alpha_airflow(airflow_align)
    ...
    if airflow_align.ndim > 1:
        ...
        Ft_align_x = CV.dp(altitude, airflow_align)[:, None] * S_BODY * CV.Ct(alpha, roll)[:, None] * direction_x
    else:
        ...
        Ft_align_x = CV.dp(altitude, airflow_align) * S_BODY * CV.Ct(alpha, roll) * direction_x

    return Ft_align_x


def Tn_align(time, altitude, airflow_align, roll):

    alpha = CV.alpha_airflow(airflow_align)
    #alpha = CV.alpha(airflow_align)
    Cp = Cp_body(time, airflow_align, roll)
    Fn = Fn_align(altitude, airflow_align, roll)

    Tn_align = cross(Cp, Fn)

    return Tn_align

def Tt_align_roll_control(altitude, airflow_align, delta):

    if airflow_align.ndim > 1:

        direction_x = np.array([1, 0, 0])
        Tt_align_roll_control = np.sign(delta)[:, None] * CV.dp(altitude, airflow_align)[:, None] * S_BODY * D_BODY * CV.Cmt_roll_control(delta)[:, None] * direction_x

    else:

        direction_x = np.array([1, 0, 0])
        Tt_align_roll_control = np.sign(delta) * CV.dp(altitude, airflow_align) * S_BODY * D_BODY * CV.Cmt_roll_control(delta) * direction_x

    return Tt_align_roll_control
...
if __name__ == '__main__':
    ...
    print(Tn_align(time, altitude, airflow_align, 0))
